...
def simulate_meteor_atmospheric_entry(diameter_m, velocity_m_s, entry_angle_deg, density_kg_m3=None):
    """
    Simulates the atmospheric entry of a meteor and calculates its final energy.

    Parameters:
        diameter_m: Diameter of the meteor (m)
        velocity_m_s: Initial velocity (m/s)
        entry_angle_deg: Entry angle from horizontal (degrees)
        density_kg_m3: Meteor density (kg/m³). If None, uses default rock density (3000 kg/m³)
...
if __name__ == "__main__":
    ...
    print("\n--- Example with custom density (NASA NEOWs conversion) ---")
    density_from_nasa = 2.5  # g/cm³ from NASA API
    density_kg_m3 = density_from_nasa * 1000  # Convert to kg/m³
    
    d = 300
    Ef, v_final, mass, E_drag, percent_lost = simulate_meteor_atmospheric_entry(
        d, velocity_example, entry_angle_deg, density_kg_m3
    )
    print(f"Diameter: {d:.0f} m | Density: {density_kg_m3:.0f} kg/m³ | Mass: {mass:.2e} kg")
    print(f"Final velocity: {v_final:.1f} m/s | Ef: {Ef:.2e} J | Energy lost: {E_drag:.2e} J ({percent_lost:.2f}%)")